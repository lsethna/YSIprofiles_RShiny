...
ui <- fluidPage(
  titlePanel("YSI profiles: Clip and plot"),
  tabsetPanel(
    tabPanel("Instructions",
             p("1. Ensure the column names in your YSI profile data match the metadata table below."),
             p("2. On the 'Upload and select data' tab, upload the YSI profile data. Only .csv files will work!"),
    ...
    tabPanel("Upload and select data", 
             fileInput("file", "Upload CSV File"),
             actionButton("loadData", "Load Data"),
             tableOutput("loadData"),
             selectInput("site",label="Site",choices=unique(loadData))
             ),
...
server <- function(input, output, session) {
  ...
  output$meta_dat <- renderTable({
    meta_dat
  })
  #Render the dataframe for the upload data tab
